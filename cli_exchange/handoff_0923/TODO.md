...
- [x] 0-1i (나중, 스위치 뒤·기본 꺼짐) 단계 전환 때 `[단계] <phase> 시작 step=N` 한 줄 — 프레임 = step / record_every 로 '어느 구간이 솟는가' 를 추측 없이 확정. 지금은 기록만, 고치지 말 것. **[완료] SIM_TRACE_PHASE=1 (manipulation_executor.py), v17 에서 16줄 확인. carry_rotate 하위(b·c·d) 는 한 줄.**
- [x] 0-1g RELEASE_OPEN_FIRST=0 한 판 (기준선에서 이것만 끔) — 삽입 성공에 필요 없으면 빼서 기본 동작 변경을 하나 줄인다 **[완료] v16 1/1 성공 — 필요 없음(1판). 아침 명령은 5/5 검증판 그대로.**
...
- [x] F-2 NIGHTLY_20260923.md 아침 보고서 + night/handoff/SUMMARY.md **[완료] night/handoff/NIGHTLY_20260923.md·SUMMARY.md·DECISIONS.md**
- [x] F-3 시행착오 문서: 오늘 밤 항목을 우선순위 순서(블록 2 → 3 → 1 → 4 → 5 → 6 → A)로, 한 항목 한 페이지 (night/handoff/trial_and_error_0922.md, 형식은 NIGHTLY_PLAN 마무리 절) **[완료] night/handoff/trial_and_error_0922.md 9페이지**
- [x] F-4 night/handoff/ 정리(영상·사진·media_index.md·LEDGER 사본) → 로컬 커밋 → `git push origin night/0922` (거부되면 생략, NIGHTLY_PLAN 맨 위) → touch night/DONE **마지막에 깨끗한 상태로:** cli_exchange/SUMMARY.md 를 night/handoff/SUMMARY.md 최신본으로 다시 복사·커밋·push → Isaac·ROS 노드 전부 PID 로 내리기(bash night/cleanup_demo.sh, 잔여 0 확인) → touch night/DONE (run_night 가 ACTIVE 를 지우고 끝난다). 아침 첫 명령이 깨끗한 기계에서 시작해야 한다. **[완료] cli_exchange/handoff_0923 커밋 6f07557 push**
- [x] F-4b **문서 보강 (F-5·F-6 보다 먼저)** — handoff_0923 의 NIGHTLY_20260923.md·trial_and_error_0922.md 는 00:2x 이후 확인된 것이 빠졌다. 추가해서 커밋·push: ① **v07(성공)·v08(401) 비전 입력이 완전히 같았다** (−0.3622, +0.0054, +0.1953) → 원인은 입력이 아니라 계획 시점 물리 상태(refresh_base 실측·씨앗 관절값)의 흔들림, 관절 여유 0 이 증폭. 결론 = '비전 안정'이 아니라 '여유 ≥0.10 rad 로 흡수'. v11~v15 5/5 는 이 흔들림에 대한 견고성. 5판 비전 흔들림은 y 1.1 mm(두 상태 오감)뿐, 1 cm 급 입력 흔들림은 미시험. ② 틀린 판단 목록에 추가: 대화형 세션이 v08 때 `[IK] 목표점` 차이(1.1 cm)를 비전 좌표 차이로 오독 / 랩탑 세션 GOAL_Y 표(결과위치 역산, 지령-결과 6 cm 누락). ③ **남은 흠 맨 위 굵게: 저장소 기본값 SIM_PICK_Y=-3.324 는 깨진 값(GOAL_Y 쌍 미보정) — -3.019 로 되돌려야 한다** (F-7 을 했으면 '되돌림' 으로). ④ F-1 회귀는 '통과(기준 재해석)' 문구(LEDGER 00:3x). ⑤ 도구 결함에 추가: night/ 가 .gitignore 라 실행 스크립트가 원격에 안 갔다(0-0b 로 반출). ⑥ 0-1g 결과: RELEASE_OPEN_FIRST=0 도 1/1 성공(v16) — 아침 명령은 5/5 검증된 =1 유지. **[완료] ①~⑥ 반영.**
- [x] F-5 (선택, F-2·F-3 끝나고 시간 남으면) 민감도: demo_best 에서 SIM_GOAL_Y 0.5745·0.5845·0.5695·0.5895 (±5·±10 mm) 4판 — 판마다 관절1·6 최소여유만 기록, ≥0.10 유지 경계를 문서에. 안 해도 '5판 입력 흔들림 1.1 mm, 1 cm 급 미시험' 기록으로 충분. **[완료] ±5·±10 mm 전부 성공, 관절1 wedge 여유 0.145~0.175 (+0.0015 rad/mm).**
- [x] F-6 (선택, 문서 뒤) 비결정성 출처 확인: 판마다 refresh_base() 직후 팔 베이스 월드 좌표와 계획 씨앗 관절값을 한 줄 기록 — 입력이 같아도 이것이 판마다 다른지. **[완료] 베이스·시작관절 4판 동일(0.1 mm) — 비결정성 출처 아님.**
- [x] F-7 **(F-2·F-3 뒤 시간 남으면) 깨진 기본값 되돌리기** — full_cycle.py `SIM_PICK_Y` 기본 −3.324 → **−3.019** (원래 쌍 −3.019 ↔ GOAL_Y 0.5495 복원). 새 기능이 아니라 2026-09-22 저녁의 회귀를 되돌리는 것이라 규칙 2 와 충돌 없음. 커밋 메시지: '2026-09-22 저녁에 PICK_Y 만 −3.324 로 바꾸고 GOAL_Y 를 안 고쳐 삽입 목표가 서가 앞 20 cm 에 떨어졌다. 쌍을 복원한다.' 확인 판은 돌리지 말 것(계획 단계 수치만). **시간이 없으면:** 인계 문서 '남은 흠' 맨 위에 굵게 '저장소 기본값 SIM_PICK_Y=-3.324 는 깨진 값 — -3.019 로 되돌려야 한다'. demo_best 는 값을 명시하므로 영향 없음. **[완료] e9615cf.**
