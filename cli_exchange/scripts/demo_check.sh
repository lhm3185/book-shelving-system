...
echo "  스캔       $(grep -oP 'scan_command:\s*\K\S+' ros2_ws/src/shelving_manipulation/config/manipulation.yaml 2>/dev/null || echo '?')  (비전팀 구현은 scan_sweep)"
echo "  ※ 스윕이 아래 판에서 안 풀리면 로그에 '[스윕] 판 … 정지점 0/N' 이 찍힌다."
echo "    그때 옛 동작으로 한 판 보려면:  MAN_EXTRA='-p scan_command:=scan_shelf' 를 앞에 붙인다"
