...
class MotionConfig:
    ...
    # --- 자세 -------------------------------------------------------------
    #: **두 홈이 있다.** 이 값은 장면 yaml 의 것이고, 실제 판은 `SIM_HOME_Q` 로
    #: 갈아 낀 자세로 돈다. 2026-09-24 에 내가 이걸 실제 값으로 알고 오프라인 재현을
    #: 했다가 **실제와 다른 답을 얻고 그것을 근거로 단언했다.**
    #:   yaml 홈   파지 여유가 작다(전 구간 최소 0.128) · **아래 판 스윕이 풀린다**
    #:   SIM_HOME_Q 파지 여유가 크다(0.707/0.389/0.282) · **아래 판 스윕이 안 풀린다**
    #: 둘 다 쓰인다. 어느 하나를 "그 홈" 이라고 부르면 틀린다.
    q_home: Sequence[float] = (2.811, -0.514, 0.102, -2.213, 0.051, 1.701, -1.033)
    #: 실제 판이 쓰는 홈 (`SIM_HOME_Q`). 실행 셸에만 있던 값을 여기 적어 둔다 —
    #: 오늘 같은 사고가 **네 번째**다 (맞춤 둘 · scan_command · 이것).
    q_home_sim: Sequence[float] = (-2.3634, -0.73, -0.6933, -2.1884, -0.4396, 1.5845, -0.554)
