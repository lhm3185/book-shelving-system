...
#: **(2026-09-23) 기본을 −3.019 에서 −3.0695 로 옮긴다.** 근거는 여유 곡선 실측이다.
#: 팔 기준 goal_y 를 늘리는 것과 베이스를 뒤로 빼는 것은 팔에게 같은 일이라
#: (짝 규칙의 뜻), 그 곡선이 곧 AMR 도착 오차에 대한 내성이다. 곡선은 완만하지 않다:
#:   goal_y ≲ 0.564 → IK 가 관절6 가지로 떨어져 여유 ≈ 0
#:   0.562~0.564   → 1 mm 단위로 가지가 뒤집히는 **쌍안정** 구간
#:   goal_y ≳ 0.565 → 관절1 가지, 안정
#: 옛 기본(−3.019)에서 AMR 이 서가 쪽으로 30 mm 더 가면 goal_y 0.5495 가 되고,
#: 실측 여유가 **0.000 rad (관절6)** 이다 — 성공으로 끝나도 한계에 붙어 돈 것이다
#: (실제로 야간 v07 ✓ / v08 401 / v09 ✓ 로 갈렸다).
#: 새 기본에서 같은 30 mm 오차는 goal_y 0.570, 실측 여유 **0.157 rad (관절1)** 이다.
#: 측정: docs/doyoon-kim/measurements/20260923_base_offset_runs.txt
#: 주의: GOAL_Y 는 따로 적지 않는다 — pick_from_vision 이 짝 규칙으로 만든다.
PICK_SPOT = (2.535, float(os.environ.get("SIM_PICK_Y", "-3.0695")))
#: 거기서 바라볼 방향 (월드 yaw, 도).
