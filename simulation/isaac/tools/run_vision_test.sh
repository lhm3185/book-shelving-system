...
source "$HOME/ws_cobot_pjt/book-shelving-system/ros2_ws/install/setup.bash"
...
ros2 run shelving_perception vision_manager --ros-args -p use_sim_time:=true \
    -p model_path:="${MODEL_PATH:-$HOME/ws_cobot_pjt/arm/models/book_best.pt}" -p target_frame:="$TARGET_FRAME" \
