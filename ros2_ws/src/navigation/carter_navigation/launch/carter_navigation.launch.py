...
def generate_launch_description():
    package_share_dir = get_package_share_directory("carter_navigation")
    nav2_bringup_launch_dir = os.path.join(
        get_package_share_directory("nav2_bringup"), "launch"
    )

    default_map = os.path.join(package_share_dir, "maps", "library.yaml")
    default_params = os.path.join(
        package_share_dir, "params", "carter_navigation_params.yaml"
    )
    rviz_config_dir = os.path.join(
        package_share_dir, "rviz2", "carter_navigation.rviz"
    )
    ...
    return LaunchDescription(
        [
            ...
            Node(
                ...
                executable="sensor_stamp_to_clock.py",
