...
왜 있나 (2026-09-20~21)
    ...
    **모든 목표가 410(요청 오류)으로 거절**됐을 것이다. 사람이 눈으로 맞추는 대신
    ...
    그리고 지금 M0609 의 바깥쪽 칸(x = -0.2297)은 **여유가 9.7 mm 뿐**이다.
    누가 상자를 조금만 좁히면 조용히 거절이 시작된다 — 그 여유도 같이 고정한다.
"""
...
# 발행된 계약 좌표 (ros2_ws/src/shelving_manipulation/config/book_profiles.yaml 주석과 같아야 한다)
...
@pytest.mark.parametrize("p", FRANKA, ids=lambda p: f"franka{p[0]:+.4f}")
def test_franka_좌표가_검증을_통과한다(p):
    assert _inside(p), f"{p} 가 place_region 밖이다 — 410 으로 거절된다"


@pytest.mark.parametrize("p", M0609, ids=lambda p: f"m0609{p[0]:+.4f}")
def test_m0609_좌표가_검증을_통과한다(p):
    assert _inside(p), (
        f"{p} 가 place_region 밖이다 — 410 으로 거절된다. "
        "M0609 는 팔 베이스가 37cm 높아 꽂는 z 가 0.5097 이다")


def test_바깥쪽_칸의_여유가_줄지_않았다():
    """x = -0.2297 은 여유가 1cm 도 안 된다. 상자를 좁히면 여기부터 거절된다."""
    worst = min(_margin(p) for p in M0609)
    assert worst >= 0.009, (
        f"가장 좁은 여유가 {worst*1000:.1f} mm 로 줄었다. "
        "place_region 을 좁혔거나 좌표를 옮겼다면 둘이 맞는지 확인할 것")


def test_두_로봇의_z_가_같은_식에서_나온다():
    """계약 z = 선반판 + 책높이/2 + 삽입여유 - 팔베이스.

    이 식을 재현 못 하면 숫자 안에 설명 안 되는 상수가 섞인 것이다.
    ...
    for floor_z, base_z, contract_z in cases:
        ...
        assert abs(derived - contract_z) < 0.001, (
            f"선반판 {floor_z} / 베이스 {base_z} → 유도 {derived:.4f} 인데 "
            f"계약값은 {contract_z}. 차이 {abs(derived-contract_z)*1000:.1f} mm")
