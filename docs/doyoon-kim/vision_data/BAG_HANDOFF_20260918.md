...
### `tray_during_place` 기준 수치 (새 모델, 임계값 0.75, 30 프레임 표본)

| 항목 | 값 |
| --- | --- |
| 프레임당 상자 | 평균 4.37 (0 ~ 6) |
| 상자 없음 프레임 | 3 / 30 — 팔이 트레이를 가리거나 카메라가 서가를 볼 때 |
| 상자 폭 | 평균 7.1 % (최대 12.5 %) |
| 묶임 상자 | 0개 |
| 신뢰도 | 평균 0.90 |

책을 꺼내 가는 장면이라 **화면의 책 수가 계속 바뀝니다.** 검출률을 잴 때는 이 점을 감안해 주세요.
...
bag 이 zstd 로 압축돼 있어, 파이썬으로 직접 읽으면 **bag 폴더 옆에 압축을 푼 임시 파일(1.7~2.4 GB)이 만들어집니다.**
USB 에서 바로 돌리지 마시고 **하드디스크로 복사한 뒤** 돌려 주세요. `ros2 bag play` 는 이런 임시 파일을 만들지 않습니다.
