...
> **2026-09-20 — 실행 순서는 이 문서가 아니라 `DEMO_20260921.md` 를 보세요.**
> 이 문서는 Franka 시절(9/17)에 쓴 것이라 **지금 실행 방법과 다릅니다**:
> 로봇이 M0609 결합체로 바뀌었고(`ARM_ROBOT=m0609`), `--start-home snap` 이 필수가 됐고,
> 팔 기준 TF 부모가 `panda_link0` → `base_link` 로 바뀌었습니다.
> 아래 **부록의 운영 규칙(트레이 6권 한도, 5·6번 칸 신뢰도, Isaac 재시작 시 노드 재기동)은
> 지금도 유효**하므로 그 부분만 참고하세요.
...
## 부록 — 2026-09-19 실측으로 확정된 운영 규칙

### 1. 한 Isaac 세션에 트레이 책은 **6권**이다

| 시험 | 결과 |
| --- | --- |
| 한 세션에서 4권 (1판) | **4/4, 88초** |
| 같은 세션에서 4권 더 (2판) | **1/4** — 5번째 책만 성공, 6번째 404, 이후 411 `트레이에 남은 책이 없음` |
| 같은 세션에서 또 (3판) | **0/4**, 전부 411 |

**→ 시연 한 번 = Isaac 한 세션.** 다시 보여 달라는 요청이 오면 **Isaac 을 재시작**해야 트레이가 다시 채워진다.

### 2. 트레이 **5·6번 칸은 신뢰도가 낮다**

2판에서 나온 실패가 그 칸들이다.

| 코드 | 메시지 |
| --- | --- |
| 404 | `job > down: 제한 시간 초과 (429 스텝)` |
| 409 | `배치 확인 실패 {'upright': False, ...}` |

**1차 시연은 4권이라 앞 4칸만 쓴다 — 안전 범위 안이다.**
5권 이상을 보여 달라는 요청이 오면 이 점을 미리 말하고, 필요하면 재시작 후 4권씩 나눠 보여준다.

### 2-1. 반복 재현성 — **시연 당일 방식으로 3판 연속 4/4** (2026-09-19)

판마다 Isaac·노드를 **전부 새로 올려서** 돌렸다. 시연 당일과 같은 절차다.
...
준비 시간도 쟀다: **Isaac 16초 + 노드 2~6초.** 호출 뒤 재시작하더라도 **30초 안에 시작**할 수 있다.

**→ 간헐 실패는 관찰되지 않았다.** 다만 이것은 **앞 4칸만 쓸 때**의 값이다 (아래 2번 참조).

### 2-2. 긴 대기 후 첫 실행 — **된다. 다만 게이트 확인이 거짓 경보를 낸다** (2026-09-19)

시연 당일은 "아침에 리허설 → 호출까지 띄운 채 대기 → 호출 직후 실행" 이다. **15분 놀린 뒤**를 재 봤다.

| 확인 | 결과 |
| --- | --- |
| **첫 명령** | **성공, 26초** (평소 21~25초와 사실상 같다) |
| 대기 직후 `ros2 node list --no-daemon` | **노드가 안 보였다** — Isaac 것만 보임 |
| 노드 실제 상태 | **전부 살아 있었다** (프로세스 17분째 정상) |
| 명령을 한 번 돌린 뒤 다시 조회 | **5개 전부 정상 표시** |

**→ 대기 후에는 `ros2 node list` 를 go/no-go 기준으로 쓰면 안 된다.**
노드는 멀쩡한데 목록에 안 나온다. 이걸 모르고 보면 **멀쩡한 시스템을 재시작**하게 되고,
그러면 Isaac 까지 다시 올려야 해서 1분을 버린다.

```
대기 후 확인 순서 (권장)
  1. ros2 action info /place_book   →  Action servers: 1 이면 살아 있다
  2. 목록에 노드가 안 보여도 **당황하지 말 것** — 명령을 한 번 보내면 복구된다
  3. 정말 죽었는지는 프로세스로 본다 (ps), 토픽 목록으로 보지 않는다
```

### 3. **Isaac 을 재시작하면 이 PC 노드도 반드시 재기동한다**

2026-09-19 에 실제로 걸렸다. Isaac 만 새로 띄우고 기존 노드를 그대로 두었더니:

```
404 → 409 → 411 → 411   (4권 전부 실패)
```

노드 재기동 + `ros2 daemon stop` 후 **4/4 로 복구**됐다.

```bash
# Isaac 을 다시 띄웠다면 이 PC 에서도 반드시:
#   1) 기존 노드 종료   2) ros2 daemon stop   3) run_demo_pc.sh 다시
```

**증상이 "로봇팔이 고장난 것처럼" 보이므로 시연 중에 당하면 원인을 못 찾는다.** 순서로 고정한다.

### 4. 녹화는 **화면 녹화로 하지 않는다**

GPU PC 는 공용이라 바탕화면에 팀원 작업이 떠 있다. 실제로 화면 녹화를 했더니 **팀원의 작업 화면이 찍혀 파일을 폐기**했다.

```bash
# 시뮬 안에서 직접 찍는다 (헤드리스, 남의 화면을 건드리지 않는다)
./scripts/run_isaac_sim.sh --headless ... --record-dir /tmp/b1_frames
ffmpeg -framerate 20 -pattern_type glob -i "/tmp/b1_frames/f*.png" \
       -c:v libx264 -pix_fmt yuv420p 출력.mp4
```

카메라 구도는 로봇 AABB 로 자동으로 잡힌다. 좌표를 손으로 주면 엉뚱한 곳(서가 벽)을 찍는다.
