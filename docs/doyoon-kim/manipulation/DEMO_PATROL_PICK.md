...
| `406 운반 중 손 안에서 N cm 어긋남` | 책이 손에서 미끄러짐 | 먼저 **파지 x 스냅 크기**를 본다 (아래) — 10 mm 이상이면 트레이가 밀린 것이다 |
| `409 배치 확인 실패` (항목 전부 false) | 책이 엉뚱한 자세로 끝남 (대개 바닥) | `isaac.log` 의 `[놓음]` AABB 가 세워진 책 치수와 다르면 기울어진 채 놓인 것 |

### 건강 지표 — 파지 전에 이상을 알려준다

로그의 `파지 x 를 칸 중심에 맞췄다: … (N mm)` 값을 본다.

| N | 뜻 |
| --- | --- |
| **~0.3 mm** | 정상 |
| **10 mm 이상** | **트레이가 설정 자리에서 벗어났다** — 주행·복귀 보정 쪽을 본다 |

정상 0.3 mm / 이상 19.9 mm 로 두 자릿수 차이가 나서 눈에 바로 띈다.
실패한 뒤 원인을 캐는 것보다 훨씬 싸다.
...
- **그 보정은 트레이를 끌고 가면 안 된다.** 보정이 루트를 옮기면 트레이 앵커(차체)도
  함께 끌려가 칸 중심에 있던 책이 5.4 cm 밀린다 → 운반 중 406.
  `Scene.rebase_tray_to()` 가 트레이를 그 자리에 두고 관계만 다시 잡는다. **빼지 말 것.**
...
**이 브랜치는 1차 통합(파지 + 비전 + 주행)의 동결본이다.** 여기서는 더 고치지 않는다.
이후 작업(M406 계측, 오차 예산 측정, Nav2 인수)은 다른 브랜치에서 한다.

주행이 Nav2 로 바뀌면 복귀 보정과 `rebase_tray_to` 는 함께 없어진다 —
순간이동 때문에 생긴 문제를 막던 것이기 때문이다. 순서와 확인 항목은
`NAV2_HANDOFF_CHECKLIST.md` 에 있다. 오늘 밟은 함정은 `TRAPS_20260921.md` 에 있다.
